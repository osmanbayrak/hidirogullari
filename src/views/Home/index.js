...
import { Input, Form, notification, Button, Card, Col, Row } from 'antd';
...
class HomePage extends React.Component {
  ...
  handleSubmit = e => {
    ...
    this.props.form.validateFields((err, values) => {
      if (!err) {
        Axios.post('form-kayit', this.state).then(
          response => {
          ...
          error => {
            notification['error']({
              ...
              message: 'Hata!',
              description:
                'Malesef bir sorun oluştu. Lütfen daha sonra tekrar kayıt formunu doldurmayı deneyiniz..',
            });
          },
        );
      }
              ...
              >
                <Form.Item {...formItemLayout} label="İsim">
                  {getFieldDecorator('isim', {
                    ...
                    <Input
                      onChange={e => {
                        this.setState({ isim: e.target.value });
                      }}
                    />,
                  )}
                </Form.Item>
                <Form.Item {...formItemLayout} label="Soyisim">
                  {getFieldDecorator('soyisim', {
                    ...
                    <Input
                      onChange={e => {
                        this.setState({ soyisim: e.target.value });
                      }}
                    />,
                  )}
                </Form.Item>
                <Form.Item {...formItemLayout} label="Yaş">
                  {getFieldDecorator('yas', {
                    ...
                    <Input
                      onChange={e => {
                        this.setState({ yas: e.target.value });
                      }}
                    />,
                  )}
                </Form.Item>
                ...
                <Form.Item {...formItemLayout} label="Meslek">
                  {getFieldDecorator('meslek', {
                    ...
                    <Input
                      onChange={e => {
                        this.setState({ meslek: e.target.value });
                      }}
                    />,
                  )}
                </Form.Item>
                ...
                <Form.Item {...formSubmitLayout} style={{ marginTop: '15px' }}>
                  <Button
                    style={{ float: 'right' }}
                    type="danger"
                    htmlType="submit"
